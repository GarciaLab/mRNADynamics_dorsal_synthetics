function out=mcmcpred_eqmodel(results,chain,s2chain,data,modelfun,nsample,varargin)
...
%   ysave = zeros(nsample, size(datai.ydata, 1), size(datai.ydata, 2)-2); %AR 10/21/20 this preallocation might be wrong for some datasets. can be modified though. 
    ...
    %this definitely only works for a single y output. 
    ysave = zeros(nsample, numel(x)); %AR 10/21/20 this preallocation might be wrong for some datasets. can be modified though. 
    osave = zeros(nsample, numel(x));
