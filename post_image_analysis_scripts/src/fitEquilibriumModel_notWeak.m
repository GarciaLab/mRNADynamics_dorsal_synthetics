function [results, chain, s2chain, data, modelOpts] = fitEquilibriumModel_notWeak(varargin)
...
%mdl inputs are 1. dorsal 2. {amp, KD, omegaDP, P/KP}. Any of the elements
%of 2 are allowed to be column vectors.
mdl =  @(x, p) p(1).*(...
    (   p(4) + (x./p(2)).*p(3).*p(4)   )...
    ./( 1+ p(4) + (x./p(2))+ (  (x./p(2)).*p(3).*p(4))    )...
    );

%model.modelfun   = @(x, p) p(1).*(((x.ydata(:, 1)./p(2)).*p(3))./(1+ (x.ydata(:, 1)./p(2))+ ((x.ydata(:, 1)./p(2)).*p(3)))); %use mcmcrun generated ssfun
% model.modelfun  = @(x, p) p(1).*(...
%     (   p(4) + (x.ydata(:, 1)./p(2)).*p(3).*p(4)   )...
%     ./( 1+ p(4) + (x.ydata(:, 1)./p(2))+ (  (x.ydata(:, 1)./p(2)).*p(3).*p(4))    )...
%     );
 model.modelfun  = @(x, p) mdl(x.ydata(:, 1), p);
results = [];
...
if fixKD && batchedAffinities
    for k = 1:length(enhancers)
        yy{k} = mdl(xForPlot, [theta_mean(1), fixedKDs(k), theta_mean(3)]);
    end
else
    if ~iscell(theta_mean)
        yy = mdl(xForPlot, theta_mean);
    else
        for k = 1:length(theta_mean)
            chaink = [chain(:, 1), chain(:, k+1),  chain(:, end-2),  chain(:, end)];
%             a = 0;
%             for j = 1:size(chaink, 1)
